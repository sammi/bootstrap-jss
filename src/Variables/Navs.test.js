...
describe('Navs', () => {
  it('Navs', () => {
    ...
    expect(navTabsLinkHoverBorderColor).toEqual(`${grays._200} ${grays._200} ${navTabsBorderColor}`)
    expect(navTabsLinkActiveColor).toEqual(grays._700)
    expect(navTabsLinkActiveBg).toEqual(bodyBg)
    expect(navTabsLinkActiveBorderColor).toEqual(`${grays._300} ${grays._300} ${navTabsLinkActiveBg}`)
    ...
    expect(navDividerMarginY).toEqual(`${size(spacer).value / 2}${size(spacer).unit}`)
  })
})
