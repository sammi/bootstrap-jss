import {grays} from './Colors'
import {borderRadius} from './Components'

export const breadcrumbPaddingY = '.75rem !default'
export const breadcrumbPaddingX = '1rem !default'
export const breadcrumbItemPadding = '.5rem !default'

export const breadcrumbMarginBottom = '1rem !default'

export const breadcrumbBg = grays._200
export const breadcrumbDividerColor = grays._600
export const breadcrumbctiveColor = grays._600
export const breadcrumbDivider = 'quote("/") !default'

export const breadcrumbBorderRadius = borderRadius
